...
PATCHES=(
	"${FILESDIR}/${PN}-2.4.0-cgal-6.0.patch"
	"${FILESDIR}/${PN}-2.4.0-cgal-deprecated-headers.patch"
	"${FILESDIR}/${PN}-2.3.1-opencascade-7.8.0.patch"
	"${FILESDIR}/${PN}-2.3.1-dont-expect-opencv_world.patch"
	"${FILESDIR}/${PN}-2.4.0-boost-1.88.patch"
	"${FILESDIR}/${PN}-2.3.2-boost-1.90.patch"
	"${FILESDIR}/${PN}-2.4.0-fix-linking-to-webkitgtk.patch"
)

src_prepare() {
	cmake_src_prepare

	# Upstream requires wxWidgets 3.3 but we ship 3.2
	sed -i -e 's/find_package(wxWidgets 3\.3/find_package(wxWidgets 3.2/' \
